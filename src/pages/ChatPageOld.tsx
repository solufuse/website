...
const ChatPageOld: React.FC = () => {
    ...
    const handleNewConversation = async () => {
        ...
        if (newChat && currentProject) {
            navigate(`/chat-old/${currentProject.id}/${newChat.short_id}`);
        }
    };

    const handleConversationSelect = (id: string) => {
        if (currentProject) {
            navigate(`/chat-old/${currentProject.id}/${id}`);
        }
    };
    ...
    const handleShare = (messageId?: string) => {
        if (currentProject && activeChatId) {
            let link = `${window.location.origin}/chat-old/${currentProject.id}/${activeChatId}`;
            if (messageId) {
                link += `/${messageId}`;
            }
